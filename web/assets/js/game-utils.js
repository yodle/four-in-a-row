...
var buildSequence = function(row, col, board, dir) {
    result = [];
    if (dir == "rows") {
        for (var i = 0;  i < 4; i++ ) {
            result[i] = {
...
exports.getWinSequence = function(board) {
    if(board === undefined || board[0] === undefined) {
        return false;
    }
    ...
    for(var col=0; col<cols; col++) {
        for(var row=0; row<rows; row++) {
            for(var dir in checkBoard[col][row]) {
                ...
                if (checkBoard[col][row][dir] == 4) { 
                    return buildSequence(row, col, board, dir);
                }
            }
        }
    }
